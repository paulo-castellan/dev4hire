...
describe 'user sees projects propositions' do
  ...
  it 'sees devs profile before decides' do
    jonas = Dev.create!(email: 'dev@dev.com', password: 123456)
    expertise = Expertise.create!(field_of_expertise: 'Dev Back-end')
    dev_profile = DevProfile.create!(full_name: 'João Carlos', social_name: 'João Carlos',
                                    date_of_birth: '25/11/1987', academic_education: 'Formado em SI pela UFPE',
    ...
    expect(current_path).to eq(dev_profile_path(jonas.id))
    expect(page).to have_content('Nome completo: João Carlos')
    expect(page).to have_content('Data de nascimento: 25/11/1987')
    expect(page).to have_content('Área de atuação: Dev Back-end')
    expect(page).to have_content('Formação educacional: Formado em SI pela UFPE')
    expect(page).to have_content('Experiência profissional: Estágio com backend na XX consult')
    expect(page).to have_content('Sobre você: Desenvolvedor que curte a vida')
